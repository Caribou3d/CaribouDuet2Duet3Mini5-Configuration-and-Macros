...
for HEIGHT in ${HeightsArray[@]}; do
    for THERMISTOR in ${ThermistorsArray[@]}; do
        ...
        for SENSOR_TYPE in ${SensorArray[@]}; do
            ...
            find -L $Start_Path -name "*$HEIGHT-LGX*-$THERMISTOR_SHORT$SENSOR_SHORT-Build*.zip" -type f -not -path "$Destination_Path_zip/$Printer_Name$HEIGHT/$THERMISTOR/$SENSOR_TYPE/*" -exec mv {} $Destination_Path_zip$Printer_Name$HEIGHT/$THERMISTOR/$SENSOR_TYPE \;
            ...
            find $Start_Path -name "*$HEIGHT-LGX*-$THERMISTOR_SHORT$SENSOR_SHORT" -type d -exec cp -r  {} $Destination_Path_orig/$Printer_Name$HEIGHT/$THERMISTOR/$SENSOR_TYPE \;
        done
    done
done
